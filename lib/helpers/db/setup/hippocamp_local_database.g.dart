  ...
  Future<sqflite.Database> open(
    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        ...
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `posts` (`key` TEXT NOT NULL, `domain_key` TEXT NOT NULL, `domain_background_color_hex` TEXT NOT NULL, `nome` TEXT NOT NULL, `type` TEXT NOT NULL, `icon_url` TEXT NOT NULL, PRIMARY KEY (`key`))');
        await database.execute(
    ...
    this.changeListener,
        ...
        _postEntityInsertionAdapter = InsertionAdapter(
            ...
            (PostEntity item) => <String, Object?>{
                  ...
                  'at': item.at,
                  'within': item.within,
                  'notifications_unit': item.notificationsUnit,
                  'morning': item.morning ? 1 : 0,
                  'afternoon': item.afternoon ? 1 : 0,
                  'evening': item.evening ? 1 : 0,
                  'whole_day': item.wholeDay ? 1 : 0,
                  'notifications_active': item.notificationsActive ? 1 : 0,
  ...
  Future<List<PostEntity>> getAllPosts() async {
    return _queryAdapter.queryList('SELECT * FROM posts',
        mapper: (Map<String, Object?> row) => PostEntity(
            ...
            at: row['at'] as String,
            within: row['within'] as String,
            notificationsUnit: row['notifications_unit'] as String,
            morning: (row['morning'] as int) != 0,
            afternoon: (row['afternoon'] as int) != 0,
            evening: (row['evening'] as int) != 0,
            wholeDay: (row['whole_day'] as int) != 0,
            notificationsActive: (row['notifications_active'] as int) != 0,
            ...
            storiesJson: row['stories_json'] as String));
  }
